...
if key_insights:
    ...
    trade_yoy_analysis = key_insights.get("trade_yoy_analysis", {})
    semiconductor_analysis = key_insights.get("semiconductor_analysis", {})
    
    top_export_commodity = export_analysis.get("top_commodity", "N/A")
    top_export_yoy = export_analysis.get("top_yoy_growth", "N/A")
    top_import_commodity = import_analysis.get("top_commodity", "N/A")
    top_import_yoy = import_analysis.get("top_yoy_growth", "N/A")
    top_export_partner = trade_yoy_analysis.get("top_export_partner", "N/A")
    top_import_partner = trade_yoy_analysis.get("top_import_partner", "N/A")
    top_semiconductor_region = semiconductor_analysis.get("top_monthly_region", "N/A")
else:
    top_export_commodity = top_export_yoy = top_import_commodity = top_import_yoy = top_export_partner = top_import_partner = top_semiconductor_region = "N/A"
...
with col1:
    st.markdown(create_metric_card(
        ...
        f"YoY: {top_export_yoy:.1f}%" if isinstance(top_export_yoy, (int, float)) else str(top_export_yoy),
        "#28a745"
    ), unsafe_allow_html=True)
with col2:
    st.markdown(create_metric_card(
        ...
        f"YoY: {top_import_yoy:.1f}%" if isinstance(top_import_yoy, (int, float)) else str(top_import_yoy),
        "#007bff"
    ), unsafe_allow_html=True)
with col3:
    st.markdown(create_metric_card(
...
with col4:
    st.markdown(create_metric_card(
        "🔌 Top Semiconductor Region",
        top_semiconductor_region,
...
if not wsts_trend_monthly.empty:
    ...
    st.markdown("""
    <div class="alert-box">
        <h4>⚠️ No Semiconductor Data Available</h4>
        <p>No semiconductor industry data is currently available.</p>
    </div>
    """, unsafe_allow_html=True)
...
tab1, tab2, tab3, tab4 = st.tabs([
    "🤝 Trade Partners", "📦 Trade Items", "🔌 Semiconductor", "⚖️ Trade Balance"
])
with tab1:
    col1, col2 = st.columns(2)
    with col1:
        if not export_top_partners.empty:
            ...
            st.dataframe(export_top_partners, use_container_width=True)
            csv = export_top_partners.to_csv(index=False)
            st.download_button("Download CSV", csv, "export_top_partners.csv", "text/csv")
        else:
            st.info("No export partners data available.")
    with col2:
        if not import_top_partners.empty:
            st.subheader("Import Partners")
            st.dataframe(import_top_partners, use_container_width=True)
            csv = import_top_partners.to_csv(index=False)
            st.download_button("Download CSV", csv, "import_top_partners.csv", "text/csv")
        else:
            st.info("No import partners data available.")
with tab2:
    col1, col2 = st.columns(2)
    with col1:
        if not export_top_items_by_amount.empty:
            st.subheader("Export Items by Amount")
            st.dataframe(export_top_items_by_amount, use_container_width=True)
            csv = export_top_items_by_amount.to_csv(index=False)
            st.download_button("Download CSV", csv, "export_top_items_by_amount.csv", "text/csv")
        else:
            st.info("No export items data available.")
    with col2:
        if not import_top_items_by_amount.empty:
            st.subheader("Import Items by Amount")
            st.dataframe(import_top_items_by_amount, use_container_width=True)
            csv = import_top_items_by_amount.to_csv(index=False)
            st.download_button("Download CSV", csv, "import_top_items_by_amount.csv", "text/csv")
        else:
            st.info("No import items data available.")
with tab3:
    if not wsts_trend_monthly.empty:
        st.subheader("Semiconductor Billings Trend")
        st.dataframe(wsts_trend_monthly, use_container_width=True)
        csv = wsts_trend_monthly.to_csv()
        st.download_button("Download CSV", csv, "wsts_trend_monthly.csv", "text/csv")
    else:
        st.info("No semiconductor data available.")
with tab4:
    if not trade_balance.empty:
        st.subheader("Trade Balance")
        st.dataframe(trade_balance, use_container_width=True)
        csv = trade_balance.to_csv(index=False)
        st.download_button("Download CSV", csv, "trade_balance.csv", "text/csv")
    else:
